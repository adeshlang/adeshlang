...
*Compile-Time Memory Safety • Zero Garbage Collector • 7 Execution Backends • Native JIT & AOT • GPU/MLIR Acceleration • First-Class ARC • Cross-Platform TUI Editor • SIMD & Parallel Execution • Modern Standard Library*

[Quick Start](#quick-start) • [Features & Language Tour](#features--language-tour) • [Standard Library](#standard-library) • [Execution Backends](#execution-backends) • [Package Manager (ADL)](#package-management--module-ecosystem) • [CLI Reference](#cli-reference) • [Adesh Editor](#adesh-editor) • [Language Server (ALS)](#developer-tools--ide-support-als) • [Documentation](#documentation)
...
See [docs/EDITOR.md](docs/EDITOR.md) for full editor documentation.
...
- **[Adesh Editor Guide](docs/EDITOR.md)**: Full guide to launching, configuring, and using `adesh editor`
- **[Release Engineering Guide](docs/RELEASE.md)**: Step-by-step release builds and deployment per OS, and launching GitHub releases
- **[Language Reference](docs/language-reference.md)**: Full grammar, syntax, functions, error model, and examples
- **[Type System Guide](docs/type-system-guide.md)**: Generics, type inference, sum types, numeric types, and type layout
- **[Memory Safety Guide](docs/memory-safety-guide.md)**: Ownership, borrowing, lifetimes, ARC, and Zero-GC architecture
- **[Execution Backends](docs/backends-guide.md)**: In-depth breakdown of Interpreter, VM, JIT, NJIT, AOT, WASM
- **[AdeshLang AI & Specialized LLM Guide](docs/AI_MODEL_GUIDE.md)**: Architecture, dataset pipeline, compiler validation, training, and `adesh ai` CLI integration
