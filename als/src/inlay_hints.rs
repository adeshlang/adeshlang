...
pub fn get_inlay_hints(_doc: &Document, symbols: &[SymbolInfo], range: Range) -> Vec<InlayHint> {
...
fn create_type_hint(symbol: &SymbolInfo) -> Option<InlayHint> {
    ...
    let position = Position::new(line_0, col_0 + symbol.name.len() as u32);
...
pub fn get_parameter_hints(_doc: &Document, _line: u32, _character: u32) -> Vec<InlayHint> {
...
pub fn get_return_type_hints(_doc: &Document, symbols: &[SymbolInfo]) -> Vec<InlayHint> {
    let hints = Vec::new();
