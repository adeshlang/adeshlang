...
use crate::runtime::simd::broadcast::{
    BroadcastMode, broadcast_scalar_left, broadcast_scalar_right, detect_broadcast,
};
...
pub fn array_add(left: &Value, right: &Value) -> Result<Value, String> {
    match detect_broadcast(left, right) {
        ...
        BroadcastMode::ScalarLeft => {
            ...
            Ok(Value::Array(broadcast_scalar_left(scalar, &arr, |a, b| {
                a + b
            })))
        }
...
pub fn array_mul(left: &Value, right: &Value) -> Result<Value, String> {
    match detect_broadcast(left, right) {
        ...
        BroadcastMode::Incompatible => {
            Err("incompatible types for array multiplication".to_string())
        }
    }
}
