...
fn lower_stmt(stmt: &Stmt, include_tests: bool) -> Result<LoweredStmt, String> {
    match &stmt.kind {
        ...
        StmtKind::UnsafeBlock(body) => match lower_stmt(body, include_tests)? {
            ...
            LoweredStmt::Skip => Err("Test function not allowed directly inside unsafe block".into()),
        },
...
fn value_to_hir_literal(val: &Value) -> HirLiteral {
    match val {
        ...
        Value::LazyRange(..) => HirLiteral::Null,
        _ => HirLiteral::Null,
    }
}
...
fn type_from_annotation(ann: &str) -> HirType {
    match ann {
        ...
        s if s.starts_with("Array<") => {
            ...
            HirType::Array(Box::new(type_from_annotation(inner)), ArrayKind::Dynamic)
        }
        s if s.starts_with("[") && s.ends_with("]") => {
            // Parse [T], [T;N], [T;raw], [T;N;raw]
        ...
        s if s.starts_with("Dict<") || s.starts_with("Map<") => {
