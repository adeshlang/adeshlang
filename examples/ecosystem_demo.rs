...
use adeshlang::stdlib::{adesh_core, adesh_alloc, EcosystemMetadata};
...
fn demonstrate_adesh_core() {
    ...
    println!("  Layout of u64: size={}, align={}", layout.size(), layout.align());
    ...
    // Intrinsics
    println!("  size_of::<u32>() = {}", adesh_core::intrinsics::size_of::<u32>());
    println!("  align_of::<u16>() = {}", adesh_core::intrinsics::align_of::<u16>());
}
...
fn demonstrate_adesh_alloc() {
    ...
    drop(arc1);
    drop(arc2);
    println!("  After dropping strong refs, upgrade: {:?}", weak.upgrade());
    ...
    vec.push(3);
    println!("  Vec: {:?} (len={}, cap={})", 
        vec.get(0).unwrap(), vec.len(), vec.capacity());
    ...
    println!("  HashMap: language={:?}, memory={:?}", 
        map.get(&"language"), map.get(&"memory"));
