...
mod tests {
    ...
    fn test_hidden_class_consistency() {
        ...
        assert_eq!(c1_x, c2_x, "Hidden class for {{x}} must be shared across objects");
        assert_eq!(c1_xy, c2_xy, "Hidden class for {{x, y}} must be shared across objects");
        ...
        assert_eq!(class.get_offset("z"), None, "Unknown property 'z' should have no offset");
        ...
        assert_ne!(c1_xy, c3_yx, "Objects with different property orders must have distinct hidden classes");
    ...
    fn test_inline_cache_monomorphic() {
        ...
        for _ in 0..10 {
            ...
            assert_eq!(cached, Some(prop_offset), "Cached property offset should match");
        }

        let (hits, misses, hit_rate): (u64, u64, f64) = ic.stats();
        assert_eq!(hits, 10);
        assert_eq!(misses, 1);
        assert!((hit_rate - (10.0f64 / 11.0f64)).abs() < 1e-6, "Hit rate should be 10/11 (~90.9%)");
        ...
        assert_eq!(wrong_class_access, None, "Lookup with mismatched class ID must miss");
    }
    ...
    fn test_constant_folding() {
        ...
        assert_eq!(folded_add, 14, "Multiplication precedence with addition must fold to 14");

        let folded_bitwise = (0xFF & 0x0F) | 0x80;
        assert_eq!(folded_bitwise, 0x8F, "Bitwise constant operations must fold to 0x8F");

        let folded_shift = (1 << 4) >> 2;
        assert_eq!(folded_shift, 4, "Bit shift constant operations must fold to 4");
    }
}
